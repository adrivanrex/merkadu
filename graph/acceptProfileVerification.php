...
$roleCheck = $users->roleCheck($username);
	if($roleCheck[0]["role"] == "admin"||$roleCheck == "moderator"){
		...
		$acceptProfileVerification = $users->acceptProfileVerification($profileToVerify);
		//echo json_encode($verifyID);
		if($acceptProfileVerification == 1){
			$data = (object) array('login' => 0,"status" => $acceptProfileVerification,"message" =>"success");
